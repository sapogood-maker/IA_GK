...
(Phase 2/3, G2B/G2C/G2D/Cognitive Quality).
...
`cognitive_core_summary` (G2C) e `cognitive_quality` (Fase 3) -
observabilidade/analise puras, nenhuma decisao nova, nenhuma chave
existente alterada.
...
build_cognitive_report()/analyze_cognitive_quality() consomem diretamente
(sem recomputar a cadeia).
...
todas de uma vez (mesmo try/except) - uma falha em qualquer parte descarta
todas, nunca um artifact parcialmente enriquecido.
"""
...
class CognitiveRunnerStage:
    ...
    `cognitive_core_summary`/`cognitive_quality` no artifact.json ja
    existente."""
